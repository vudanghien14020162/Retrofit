...
public class MainApi extends AppCompatActivity {
    ...
    private TextView tvShowListUserPage, tvUserData, tvToken;
...
//                    Integer id = date.getId();
        ...
        //post register
        final Register register = new Register("abc@gmail.com", "sad");
        mApiClient.postRegister(register).enqueue(new Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                if(response.isSuccessful()){
                    try {
                        String token = new String(response.body().bytes());
